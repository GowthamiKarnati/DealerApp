...
const PanCardUpdate = () => {
  ...
  const imageUrl =
    'https://sc0.blr1.cdn.digitaloceanspaces.com/article/126722-ebpzzqejgp-1567498856.jpeg';
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const updateResponse = await axios.post(
        'https://backendforpnf.vercel.app/updatepanphoto',
        data,
        {
          headers: {
      ...
      <View style={styles.formContainer}>
        <View style={styles.uploadContainer}>
          ...
          {panCardImage ? (
            ...
            customerKYCData['PAN Card'] &&
            customerKYCData['PAN Card'].length > 0 ? (
            // If PAN card data is available in customerKYCData, display the PAN card image
            <View style={styles.imagePreviewContainer}>
              <Image
                source={{
                  uri:
                    JSON.parse(customerKYCData['PAN Card'])[0]?.fullpath ||
                    imageUrl,
                  headers: {
                    Accept: '*/*',
                  },
                ...
                resizeMode={'cover'}
                onError={error =>
                  console.error('Error loading PAN card image:', error)
                }
              />
            </View>
          ) : null}
