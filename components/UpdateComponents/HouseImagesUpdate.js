...
const HouseImagesUpdate = () => {
  ...
  const record_id = customerKYCData.record_id;
  const imageUrl =
    'https://www.shutterstock.com/image-vector/home-vector-image-be-used-600nw-255682306.jpg';
  const [houseImages, setHouseImages] = useState([]);
  const [loading, setLoading] = useState(false);
  ...
  const handleUpload = async () => {
    try {
      ...
      const updateResponse = await axios.post(
        'https://edfd-2409-40f0-115a-ea3b-5812-e4f0-72ca-f6b6.ngrok-free.app/updatehouseimages',
        data,
        {
          headers: {
            'Content-Type': 'application/json',
