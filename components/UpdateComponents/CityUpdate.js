...
const CityUpdate = ({house}) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>{ !house ? t('updateCityTitle') : t('updateAddressTitle')}</Text>
      <View style={styles.formContainer}>
        <View style={styles.formGroup}>
          <Text style={styles.label}>{ !house ? t('city') : t('houseadress')}</Text>
          <TextInput
            editable
            multiline
            numberOfLines={4}
            ...
            onChangeText={(text) => !house ? setCity(text) : setHouseaddress(text)}
