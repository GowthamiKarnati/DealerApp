      ...
      if (userMobileNumber) {
        modifiedMobileNumber =
          userMobileNumber.length > 10
            ? userMobileNumber.slice(-10)
            : userMobileNumber;
        console.log("in home component", modifiedMobileNumber)
        const response = await axios.get(
          `https://backendforpnf.vercel.app/pnfcustomers?criteria=sheet_95100183.column_242.column_238%20LIKE%20%22%25${modifiedMobileNumber}%22`,
        );
        setData(response.data.data || []);
        setLoading(false);
      } else {
