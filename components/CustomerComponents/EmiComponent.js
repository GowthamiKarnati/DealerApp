...
const EmiComponent = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `https://backendforpnf.vercel.app/emi?criteria=sheet_26521917.column_35.column_87%20LIKE%20%22%25${encodeURIComponent(
        ...
        const sortedActiveLoanIds = Object.keys(groupedActiveTables).sort(
          (a, b) => parseInt(b) - parseInt(a),
        );
        const sortedClosedLoanIds = Object.keys(groupedClosedTables).sort(
          (a, b) => parseInt(b) - parseInt(a),
        );
        ...
        sortedActiveLoanIds.forEach(loanID => {
          const hasUnpaidEmi = groupedActiveTables[loanID].some(
            emi =>
              emi.paymentStatus.toLowerCase() === 'unpaid' ||
              emi.paymentStatus.toLowerCase() === 'bounced',
          );
          if (hasUnpaidEmi) {
        ...
        sortedClosedLoanIds.forEach(loanID => {
          const allEmisPaid = groupedClosedTables[loanID].every(
            emi => emi.paymentStatus.toLowerCase() === 'paid',
          );
          if (allEmisPaid) {
  ...
  return (
    <View style={styles.container}>
      ...
      {!loading &&
        activeLoanTables &&
        Object.entries(activeLoanTables)
          .sort(([a], [b]) => parseInt(b) - parseInt(a))
          .map(([loanID, tableData]) => (
            <View key={loanID} style={styles.tableContainer}>
              <Text style={styles.loanIdText}>
                {t('loanaccount')} {loanID}
              </Text>
              <Table
                borderStyle={{
                  borderWidth: 1,
                    ...
                    data={[
                      ...
                      <Text
                        ...
                        style={{
                          ...styles.text,
                          color:
                            rowData.statusColor === 'green' ? 'green' : 'red',
                          fontSize: fontsize,
                          fontWeight: '400',
                          margin: 6,
                          padding: 10,
                        ...
                        {t(
                          `paymentStatus.${rowData.paymentStatus.toLowerCase()}`,
                        )}
                      </Text>,
                    ]}
                    style={styles.row}
      ...
      {!loading &&
        closedLoanTables &&
        Object.entries(closedLoanTables)
          .sort(([a], [b]) => parseInt(b) - parseInt(a))
          .map(([loanID, tableData]) => (
            <View key={loanID} style={styles.tableContainer}>
              <Text style={styles.loanIdText}>
