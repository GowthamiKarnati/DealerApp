    ...
    axios.get(`https://backendforpnf.vercel.app/emi?criteria=sheet_26521917.column_35.column_242.column_238%20LIKE%20%22%25${modifiedMobileNumber}%22`)
      .then(response => {
        if (Array.isArray(response.data.data)) {
          const filteredEmis = response.data.data.filter(emi => emi.status === 'paid' || emi.status === 'bounced');
          setEmiData(filteredEmis);
        } else {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      {loading ? (
        ...
        <>
          {emiData.length > 0 ? renderEmiCards() : <Text style={styles.noDataText}>No past Emi's</Text>}
        </>
      )}
    </ScrollView>
  );
